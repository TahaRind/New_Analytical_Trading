    ...
    if st.button("Create New Analysis"):
        if analysis_name:
            ...
            try:
                ...
                st.session_state.stock_analyser_obj = create_analysis(analysis_name.upper(), initial_indicators, False)
                st.success(f"New analysis '{analysis_name}' created successfully for {analysis_name.upper()}.")
            except Exception as e:
